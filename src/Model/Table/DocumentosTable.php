...
class DocumentosTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ->scalar('nm_arquivo')
            ->maxLength('nm_arquivo', 100)
            ->requirePresence('nm_arquivo', 'create')
            ->notEmptyString('nm_arquivo');

        $validator
            ->scalar('tp_edital')
            ->maxLength('tp_edital', 100)
            ->requirePresence('tp_edital', 'create')
            ->notEmptyString('tp_edital');

        $validator
            ->date('dt_public')
            ->requirePresence('dt_public', 'create')
            ->notEmptyDate('dt_public');
